...
//static uint8_t pulce1 = 0;
//static uint8_t pulce2 = 0;
//static uint8_t pulce3 = 0;
...
/*	
	...
	if(pulce1-- == 0){						//check pulse
		gpio_set_level(AC_pin1, 0);			//turn optocupler off
	}
	if(pulce2-- == 0){
		gpio_set_level(AC_pin2, 0);
	}
	if(pulce3-- == 0){
		gpio_set_level(AC_pin3, 0);
	}
	if(fireCNT-150 == 1000 - dim1 ){ 		//150 period delay between zero cross detection and the real zero cross
		...
		pulce1 = 20;						//time optocupler on in periods
	}
